...
public:
	...
	*/
	...
	List<T> ordenaLista(const List<T> &lista) {
		Iterator it1 = _prim; // El iterador comienza en el primer elemento de la lista
		Iterator it2 = lista._prim; // El iterador comienza en el primer elemento de la lista parametrizada
		List<T> listaOrdenada;

		while (it1 != NULL && it2 != NULL) {
			if (it1.elem() <= it2.elem()) {
				listaOrdenada.push_back(it1.elem());
				it1.next();
			}
			else {
				listaOrdenada.push_back(it2.elem());
				it2.next();
			}
		}

		// En cuanto se acaban los elementos de una lista, acabamos de completar con los de la otra:
		if (it1 == NULL) {
			while (it2 != NULL) {
				listaOrdenada.push_back(it2.elem());
				it2.next();
...
int main() {
	...
	for (int i = 0; i < testcases; i++) {
		...
		// Cargamos los datos de la primera lista:
		getline(cin, line);
		...
		// Cargamos los datos de la segunda lista:
		getline(cin, line);
		...
		List<int> listaOrdenada = lista1.ordenaLista(lista2);

		// Como en cada iteración vamos a sacar el elemento ya mostrado por pantalla de la lista,
		// el tamaño inicial de la lista debemos guardarlo en una variable para evitar que se modifique
		// en cada iteración:
		int size = listaOrdenada.size();

		// Como el programa puede trabajar con listas vacías, tenemos que comprobar si efectivamente
		// hay algún elemento en la lista ordenada, o nos devolverá una excepción al intentar mostrar
		// un elemento que no existe:
		if (size > 0) {
			// Mostramos la lista final ya ordenada:
			for (int i = 0; i < size - 1; i++) {
				cout << listaOrdenada.front() << " ";
				listaOrdenada.pop_front();
			}

			cout << listaOrdenada.front() << endl;
			listaOrdenada.pop_front();
		}
	}
