...
bool esValioso(Arbin<int> arbol, int &sum);
...
int main() {
	...
	for (int i = 0; i < testcases; i++) {
		fillArbinPreorden(arbol);

		if (!arbol.esVacio()) {
			if (esValioso(arbol, sum)) {
				cout << "SI" << endl;
			}
			else {
				cout << "NO" << endl;
...
void fillArbinPreorden(Arbin<int> &arbol) {
	...
	if(n != -1) {
		fillArbinPreorden(hijoIz);
		fillArbinPreorden(hijoDr);
...
bool esValioso(Arbin<int> arbol, int &sum) {
	...
	if (hijoIz.esVacio() && hijoDr.esVacio()) {
		sum = arbol.raiz();
		return sum >= arbol.numNodos();
	}
	else if (hijoIz.esVacio()) {
		esCierto = esValioso(hijoDr, sumDr);
		sum = arbol.raiz() + sumDr;
		return (esCierto && sum >= arbol.numNodos());
	}
	else if (hijoDr.esVacio()) {
		esCierto = esValioso(hijoIz, sumIz);
		sum = arbol.raiz() + sumIz;
		return (esCierto && sum >= arbol.numNodos());
	}
	else {
		esCierto = esValioso(hijoIz, sumIz) && esValioso(hijoDr, sumDr);
		sum = arbol.raiz() + sumIz + sumDr;
		return (esCierto && sum >= arbol.numNodos());
	}
